...
/* min-width:0 defeats the CSS-grid auto-minimum: without it a grid item refuses
   to shrink below its content's intrinsic size, and an embedded Plotly chart
   extrudes the tile past its track (375px gate: 386px overflow on stock_*.html).
   overflow:hidden clips the plot while it settles instead of letting it push. */
...
@media(max-width:560px){
  ...
  /* flex-wrap:nowrap is what makes the overflow-x:auto above actually DO something.
     The base nav rule sets flex-wrap:wrap; without overriding it here the bar wrapped
     to 3 lines (100px tall) and the scroll container never had anything to scroll --
     the overflow-x had been dead code since it was added. Hide the scrollbar so the
     strip reads as a nav, not as a scroll area; the fade at the right edge is the
     affordance instead. */
  ...
  nav a{padding:6px 9px;font-size:12.5px;white-space:nowrap;flex:0 0 auto}
  .hero{padding:3rem 0 2.25rem}
  /* h1 bottoms out at the clamp() minimum (30px) long before 375px, which is where
     it starts reading as oversized against a 343px content column. Step the heading
     scale down one notch; body copy is left alone. */
  .hero h1{font-size:26px;line-height:1.22}
  .lede{font-size:15.5px}
  .block>h2{font-size:19px}
  .report h1{font-size:26px}.report h2{font-size:20px}.report h3{font-size:16px}
